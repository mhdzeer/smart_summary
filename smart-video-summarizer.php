...
function svs_settings_page()
{
    if (isset($_POST['save_svs_templates'])) {
        ...
        if (isset($_POST['template_names'])) {
            ...
            $prompts = $_POST['template_prompts'];
            foreach ($names as $i => $name) {
                if (!empty($name)) {
                    $templates[] = [
                        'name' => sanitize_text_field($name),
                        'prompt' => wp_kses_post(wp_unslash($prompts[$i]))
                    ];
                }
            }
        }
        ...
        <form method="post">
            <div id="templates_list">
                <?php foreach ($templates as $idx => $t): ?>
                    <div
                        style="background:#fff; padding:20px; border:1px solid #ccc; margin-bottom:15px; border-radius:8px; position:relative;">
                        <label><b>اسم القالب:</b></label><br>
                        <input type="text" name="template_names[]" value="<?php echo esc_attr($t['name']); ?>"
                            style="width:100%; margin:10px 0;"><br>
                        <label><b>نص التعليمات (استخدم [URL] لمكان الرابط):</b></label><br>
                        <textarea name="template_prompts[]"
                            style="width:100%; height:150px;"><?php echo esc_textarea($t['prompt']); ?></textarea>
                        <button type="button" onclick="this.parentElement.remove()"
                            style="color:red; cursor:pointer; background:none; border:none; position:absolute; left:10px; top:10px;">❌
                            حذف</button>
                    </div>
                <?php endforeach; ?>
            </div>
            <button type="button" onclick="addTemplate()" class="button" style="margin-bottom:20px;">➕ إضافة قالب
                جديد</button><br>
...
function svs_add_v4_metabox()
{
    foreach (['post', 'page'] as $s)
        add_meta_box('svs_mb', '💠 مقال النشر والطباعة الاحترافي (V 6.0)', 'svs_v4_html', $s, 'normal', 'high');
}
...
function svs_v4_html($post)
{
    $templates = get_option('svs_prompts_templates', []);
    ?>
    <div id="svs_container"
        style="direction:rtl; font-family:sans-serif; padding:15px; background:#f4f4f4; border-radius:10px; border:1px solid #ccc;">

        <div id="svs_conn_status"
            style="margin-bottom:12px; padding:6px 12px; background:#fff; border:1px solid #ddd; border-radius:20px; font-size:12px; display:inline-block;">
            ⏳ جاري فحص الاتصال...
        </div>

        <div style="margin-bottom:12px;">
            <label style="font-weight:bold; display:block; margin-bottom:5px;">رابط الفيديو المكتشف:</label>
            <input type="text" id="svs_url" style="width:100%; padding:10px; border:1px solid #ccc; border-radius:4px;"
        ...
        <div style="margin-bottom:15px;">
            ...
            <p style="font-size:11px; margin-top:5px;"><a href="<?php echo admin_url('admin.php?page=svs-settings'); ?>"
                    target="_blank">⚙️ إدارة القوالب</a></p>
        </div>

        <button type="button" id="svs_start_btn"
            style="width:100%; padding:15px; background:#1a73e8; color:#fff; border:none; border-radius:8px; cursor:pointer; font-weight:bold; font-size:16px;">
            🚀 ابدأ التلخيص آلياً
        </button>
        ...
        <div style="margin-top:20px;" id="svs_result_box">
            ...
            <textarea id="svs_raw_result"
                style="width:100%; height:250px; padding:15px; border:1px solid #bbb; border-radius:4px; font-family: 'Amiri', serif; font-size:18px; line-height:1.6; background:#fff;"></textarea>
            <button type="button" id="svs_btn_format_premium"
                style="width:100%; padding:18px; background:#2e7d32; color:#fff; border:none; border-radius:8px; cursor:pointer; font-weight:bold; font-size:18px; margin-top:15px;">
    ...
    <script>
        jQuery(document).ready(function ($) {
            ...
            $('#svs_start_btn').click(function () {
                ...
                // تبديل [URL] بالرابط الحقيقي مع ضمان عزله في سطر منفصل
                var finalPrompt = promptTemplate.replace('[URL]', "\n\n" + url + "\n\n");
                
                // إضافة تأكيد إضافي لـ Gemini للبحث عن الرابط
                finalPrompt += "\n\nملاحظة هامة: يرجى استخدام أداة YouTube للبحث عن محتوى الرابط المذكور أعلاه وتلخيصه بدقة.";
